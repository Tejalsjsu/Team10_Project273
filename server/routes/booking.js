...
router.get('/cancelBooking', function(req, res, next) {
    let cancelBooking  = "update billing set status='cancelled' where billing_id = "+req.param('billing_id');
    let errors='';
...
router.get('/getBookedMovieUserList', function(req, res, next) {
    let bookingList= [];
    //var user_id= req.session.userID;
    let getBookingDetails  = "select u.user_id, u.first_name, b.billing_id from billing b, user u where b.movie_id = 1 and b.hall_id = 1 and b.user_id=u.user_id and b.status='booked'";
    let errors='';
