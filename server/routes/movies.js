...
router.get('/getMovies', function(req, res, next) {
    ...
    let getMoviesList  = "select movieId, title, trailerLink, movieCharacters, releaseDate, photosUrl, movieLengthInMin, seeItIn, genre, minimumAge from Movies";
    let errors='';
    ...
    mysql.fetchData(function (error,results) {
        if(error){
            ...
            if(results.length > 0){
                ...
                let addMovie = "insert into movie (movieId, title, trailerLink, movieCharacters, releaseDate, movieLengthInMin, seeItIn, genre, minimumAge)";
                addMovie = addMovie + " values ('"+movie_id+"','"+req.param("title")+"','"+req.param("trailer_link")+"','"+req.param("movie_characters")+"','"+finDate+"','"+req.param("movie_length")+"','"+req.param("see_it_in")+"','"+req.param("genre")+"','"+req.param("minimum_age")+"'";
                mysql.fetchData(function (err, results) {
                    if (err) {
                        res.status(400).json(errors);
                    }
...
router.get('/getMoviesRevenue', function(req, res, next) {
    let revenueList= [];
    //var user_id= req.session.userID;
    let getRevenueList  = "select m.title, sum(b.amount)+sum(b.tax) as revenue from Movies m, Billing b where m.movieId = b.movieId and b.hallId = "+req.param("hall_id")+" group by b.movie_id";
    let errors='';
