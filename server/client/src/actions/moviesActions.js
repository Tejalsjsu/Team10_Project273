...
import { FETCH_MOVIE_LOCATIONS, FETCH_MOVIE_DETAIL } from './types';
...
export function getBookedMovieUserList(booking_details) {
  return dispatch => {
    return axios
      .get('/booking/getBookedMovieUserList', {
...
export const fetchMovieDetail = value => async dispatch => {
  const res = await axios.get(`/api/movie/details/${value}`);
  dispatch({ type: FETCH_MOVIE_DETAIL, payload: res.data });
};
